...
static void
encrypt_block( void *bc, uint8_t *outbuf, const uint8_t *inbuf )
{
...
static void
decrypt_block( void *bc, uint8_t *outbuf, const uint8_t *inbuf )
{
...
static int
do_bf_setkey( BLOWFISH_context *c, const uint8_t *key, unsigned keylen )
{
    int i, j;
	...
	((uint8_t*)&data)[1] = key[(j+1)%keylen];
	((uint8_t*)&data)[2] = key[(j+2)%keylen];
	((uint8_t*)&data)[3] = key[(j+3)%keylen];
#else
	((uint8_t*)&data)[3] = key[j];
	((uint8_t*)&data)[2] = key[(j+1)%keylen];
	((uint8_t*)&data)[1] = key[(j+2)%keylen];
	((uint8_t*)&data)[0] = key[(j+3)%keylen];
#endif
	c->p[i] ^= data;
	j = (j+4) % keylen;
    }
...
static int
bf_setkey( void *c, const uint8_t *key, unsigned keylen )
{
